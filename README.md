...
returns both trajectories plus aggregate metrics (`backend/app/api.py:401-491`).
...
| `backend/app/api.py` | Routes, `ConnectionManager`, engine registries, and the comparison pipeline (`backend/app/api.py:1-491`) |
| `backend/app/gemini_service.py` | Lazy Gemini client, `SYSTEM_PROMPT`, response schema, timeout and retry (`backend/app/gemini_service.py:1-233`) |
...
| `backend/test_engine.py` | 16 invariant tests, dependency-free (`backend/test_engine.py:1-240`) |
...
| `frontend/App.tsx` | WebSocket subscription, agent loop, run control, comparison summary table (`frontend/App.tsx:25-354`) |
| `frontend/types.ts` | Shared interfaces and the `STRATEGIES` list (`frontend/types.ts:1-105`) |
...
| `frontend/services/geminiService.ts` | `askGemini` — proxy call, actuate, log (`frontend/services/geminiService.ts:1-80`) |
...
(`backend/app/api.py:318-331`). That call is wrapped in `asyncio.to_thread`
(`backend/app/api.py:450-453`) so a comparison does not block the event loop serving the
...
`python test_engine.py` from `backend/` runs 16 invariant tests with no third-party
...
a `retryDelay` was parseable, and otherwise degrades to an `explain` response
(`:214-233`).

### 6.6 Comparison pipeline — `backend/app/api.py:266-491`

**`gemini_strategy_selector`** (`:266-281`) is the rule-based stand-in. Its docstring states
that it is not Gemini and that responses built from it are flagged via `meta.used_replay`.
...
**`build_replay_selector`** (`:284-301`) maps step to strategy and returns
`by_step.get(step)` (`:299`) — a hit switches, a miss returns `None`, which `run_steps`
...
**`build_heuristic_selector`** (`:304-315`) owns its own `min_hold = 15` gate (`:310`), so
hysteresis applies to the heuristic only.

**`summarise`** (`:333-350`) returns eleven aggregate metrics per arm.

**`_run_both_arms`** (`:352-398`) builds the control and treatment engines from the same
...
**`run_comparison`** (`:401-491`) resolves config from the DB when `replay_run_id` is given
(`:415-437`), filters the decision log to `action == "switch_strategy"` rows naming valid
strategies (`:431-436`), offloads both arms to a thread (`:450-453`), and returns
`comparison`, `summary` (with `control`, `treatment`, `delta_pct`), `meta` (including
`crn_verified`), and `diagnostics` (`:462-491`).
...
`switch_strategy` (`:41-50`), and logs **every** decision including `explain` holds with
`action` and `raw_message` carried through (`:61-71`).
...
**`ComparisonSummary`** (`frontend/App.tsx:303-352`) renders the single authoritative
...
Sources: `frontend/App.tsx:120-133`, `backend/app/api.py:401-491`,
`backend/app/api.py:352-398`.
...
| `POST` | `/api/runs/compare` | `ComparisonRequest` | `{comparison, summary, meta, diagnostics}` | `backend/app/api.py:401-491` |
...
| `ComparisonRequest` | `base_strategy="baseline"`, `steps=200`, `arrival_prob=0.6`, `mean_service=8.0`, `seed=1` | `backend/app/api.py:98-106` |
...
into `build_heuristic_selector` (`backend/app/api.py:304-315`), so it applies to the
...
(`backend/app/api.py:266-281`). 30 seeds, 500 steps
...
16 invariant tests pass (`backend/test_engine.py:1-240`), covering CRN
(`:45-79`), failure semantics (`:81-103`), replay fidelity (`:105-148`), strategy behaviour
(`:150-189`), and bookkeeping (`:191-227`).
...
| Replay at exact logged steps | [IMPLEMENTED] | `backend/app/api.py:284-301` |
| Heuristic hysteresis, scoped to the heuristic | [IMPLEMENTED] | `backend/app/api.py:304-315` |
| Comparison offloaded off the event loop | [IMPLEMENTED] | `backend/app/api.py:450-453` |
| Runtime CRN self-check surfaced to the UI | [IMPLEMENTED] | `backend/app/api.py:385-389`, `frontend/App.tsx:262-267` |
...
| Invariant test suite | [IMPLEMENTED] | `backend/test_engine.py:1-240` |
...
8. §12.2 measures a rule table, not the LLM (`backend/app/api.py:266-281`). Nothing in the
